...
> 모든 수치는 `verify_v7_final_consistency.py`(96/96)와 `v7_final_analysis/*/*.ipynb`에서 재계산된 값이다. 산식 원문은 `METHODOLOGY.md`, `지표 산정 방법론/`.
...
- 전처리: 일반 경로(정규식, 공백 구분 언어 전부) + 문자권별 추가 경로(가나 → fugashi, 가나 없는 한자 → jieba, 태국 문자 → pythainlp newmm), 영문 기능어·순수 숫자 토큰 제거.
  ...
  최종 토크나이저 소스는 저장소에 없고, 병행 구현은 `v7_final_analysis/tokenizer/`.
...
## 7. 충성도 × 파급효과 점수 (라이브 10,020건) — `METHODOLOGY.md` 2-4절
...
## 8. 보조지표 7종 (LDA와 독립, 라이브 10,020건) — `v7_final_analysis/` 각 폴더
...
| `v7_final_analysis/domestic_regional_index/domestic_regional_index_v7.json` | 국내 지역 지수 최종 재산출본(원본 라이브 JSON 미보유) |
...
python v7_final_analysis/build_notebooks_v7.py        # 노트북 8개 생성 + 실행(출력 포함)
...
python v7_final_analysis/tokenizer/jieba_and_thai_engine_details_v7.py
```
