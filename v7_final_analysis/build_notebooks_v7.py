...
실행: python v7_final_analysis/build_notebooks_v7.py            # 생성 + 실행
      python v7_final_analysis/build_notebooks_v7.py --no-exec  # 생성만
"""
import subprocess
import sys
...
OUT_DIR = Path.cwd() if (Path.cwd() / "domestic_regional_index_v7.ipynb").exists() else REPO / "v7_final_analysis" / "domestic_regional_index"
...
2. 이 노트북은 토큰 빈도(형태소 분석)를 다루지 않는다 — 10,020건 토큰 빈도 CSV는 `v7_final_analysis/tokenizer/build_bullet_token_frequency_csv_v7.py`가 산출한다.
'''),
    ]
    return cells
