...
| 핵심 모델 | **LDA 토픽(K=8) → 메타팩터(M=5) → 파급경로(F1∼F5) → 페르소나(4유형)** 4단계 해석 계층. 해석 계층은 게이트 v2(시드 분포·토픽 안정성 합의)를 통과한 **라이브 코퍼스 참고 재적합(10,020건, K=8, 시드 10개 실루엣 중앙값 0.102)** 이며, 동결 스냅샷(v7-40, 7,350건, K=10, 실루엣 0.267)은 재현되지 않는 역사 값으로 보관 |
...
경로 두 개를 고르면 페르소나가 정해진다. 이 해석 계층은 처음에는 군집 품질(실루엣)이 검증된 시점의 동결 스냅샷(v7-40)에 고정돼 있었고 이후 스물여덟 번의 재군집화가 모두 그 기준(0.267)을 넘지 못했다. 그러나 그 기준선은 저장소 재료로 어느 시드로도 재현되지 않았고(40회 적합 중 0회) 실루엣 자체가 시드에 따라 두 배 가까이 움직여, 게이트를 '단일 실루엣 임계'에서 '시드 분포·토픽 안정성 합의'(v2)로 바꿨다. v2를 통과한 최종 라이브 코퍼스의 K=8 재적합이 지금의 해석 계층이다.
...
분리하는 이원 구조를 택했다가, 기준선의 재현 불가와 시드 변동을 확인한 뒤 게이트 v2로 해석 계층을 라이브로 옮겼다.
...
|---|---|---|---|
| 라이브 코퍼스 (최종, 해석 계층 + 지표) | 10,020건 | K=8→M=5(실루엣 0.046, 시드 중앙값 0.102; 게이트 v2 통과), F1∼F5 비중, 페르소나 62/17/12/9, 전체 순위표, 팬충성도·파급효과 점수(평균 0.371/0.266), 4구획 24/17/10/49, 통계 검정, 보조지표 7종, 3D 맵 | `fandoms_v3_100.json`, `fandom_scores_live_reference_v7.json`, `chart3d_payload_live_reference_v7.json`, `analysis/persona_decision_space/live_interpretive_layer/` |
...
동결 스냅샷 순위표(최종 보고서 PDF 4절)와 상위 15 겹침 12/15, 공통 97개 팬덤 순위 Spearman 0.83. 라이브에서 god·EXO·싸이가 들어오고 RIIZE·ATEEZ·레드벨벳이 빠졌다. 전체 100개 순위는 `v7_final_10020/analysis/persona_decision_space/live_interpretive_layer/live_full_ranking_v7.csv`. 점수가 팬덤별 불릿 건수 구조에 좌우된다는 점(8절, L10·L12)은 이 표에도 그대로 적용된다.
...
라이브의 raw 요인 5개에는 동결의 '미디어노출형' 대신 '브랜드·상업형(광고·앰버서더)'이 있어 이를 F4 산업전이로 뒀다. 같은 F4·같은 페르소나명(현장상업형)이 두 계층에서 다른 raw 내용을 담는다는 뜻이다. 동결 대비 같은 페르소나를 유지한 팬덤은 공통 97개 중 49개이고, 동결 현장상업형 31개 중 16개가 라이브에서 글로벌투어형으로 옮겨갔다(이동표 `live_interpretive_layer/persona_migration_v7.csv`).

<p align="center"><img src="assets/readme/fig05_persona_map.png" alt="Fan Persona Map" width="640"></p>

*그림 5. Fan Persona Map — 팬충성도 × 파급효과 평면의 페르소나 4유형 분포(글로벌투어형 62 · 원정소비형 17 · 현장상업형 12 · 집단동원형 9). 점선은 표본 평균(0.371/0.266). 동결 판은 `fig05_persona_map_frozen_v7_40.png`.*
...
*그림 7. 페르소나를 결정하는 F1∼F5 비중 공간의 PCA(PC1 56.6%, PC2 23.1%). K→F 배정은 위 표(라이브 참고 재적합의 φ·병합 높이는 저장되지 않아 덴드로그램은 없음; 동결 K=10 덴드로그램은 `assets/readme/fig07a_persona_dendrogram_frozen_v7_40.png`). 오른쪽 패널은 F별 loading 방향이다. 홀로 떨어진 점 투어스(TWS)는 신생 그룹의 성질이 아니라 수집 언어의 결과다: r63 교체 리서치가 근거 90건 중 86건을 영문으로 적어 한국어 전용 가산 키워드·숫자 패턴에 걸리지 않고(영문 문장 EvidenceScore 평균 1.10 vs 한국어 1.78) 영문 토큰이 T3→F2로 몰려 F2 비중 0.74·합산 100위가 됐다(팬덤 전체에서 영문 본문 비중과 F2 비중의 상관 r=0.97). 그 86건을 한국어로 다시 쓴 후보 r73은 게이트 v2에 미달해 채택하지 않았다(8절). 생성: `v7_final_10020/charts/build_live_layer_figures_v7.py`(글꼴 `fonts/`). 동결 계층의 재현 노트북은 `v7_final_10020/analysis/persona_decision_space/`(K→F 10/10, 페르소나 100/100).*
...
- 해석 계층(K→F→페르소나, 전체 순위표)은 게이트 v2 채택으로 라이브 코퍼스(10,020건, K=8)로 옮겼고, 최종 보고서 PDF의 해석 계층은 v7-40 스냅샷(7,350건, K=10)이다. 두 계층은 상위 15 겹침 12/15, 같은 페르소나 49/97로 다르며 PDF와 이 README의 4·5절 수치는 일치하지 않는다. 로스터 교체 3건(BE'O→빈지노 · 한로로→몬스타엑스 · pH-1→투어스)은 라이브에만 있다. 동결 코퍼스 원본은 남아 있지 않아 `data/v7_final/frozen_snapshot_v7_40/`의 근사 복원본(97개 팬덤은 문장 집합까지 동일, 교체된 3개 팬덤은 r22 시점까지, 7,326/7,350건)으로 대신한다. 그 복원본과 재구성 토크나이저로 동결 모델을 다시 적합해도 실루엣 0.267은 재현되지 않으므로(0.07; 시드 10개 × K∈{8,10} × 코퍼스 2개 40회 적합에서도 최대 0.192, 0.267 도달 0회), 해석 계층의 수치는 재현되지 않은 스냅샷 값으로 읽어야 한다. 게이트를 '단일 실루엣 임계'에서 '시드 분포·안정성 합의'로 바꾸는 개편안(v2)과 측정된 적합 4건의 판정표는 `v7_final_10020/silhouette_gate_policy/GATE_POLICY_V2_PROPOSAL_V7.md`에 있다(v2로는 라이브 K=8만 통과; 2026-09-23 채택). 투어스(TWS) 영문 근거 86건을 한국어로 재작성한 후보 코퍼스(r73, `data/v7_final/r73_tws_ko/`)도 같은 판정표로 재봤으나 G1(시드 중앙값 0.081 < 현직 0.097)을 넘지 못해 채택하지 않았고, 해석 계층은 r72 라이브 K=8 그대로다. 그 통과 모델의 해석 계층을 같은 규칙으로 만든 것이 4·5절이며 동결과의 대조는 `v7_final_10020/analysis/persona_decision_space/live_interpretive_layer/`에 있다: 상위 15 겹침 12/15, 공통 97개 순위 ρ=0.83, 페르소나 라이브 62/17/12/9(글로벌투어·원정소비·현장상업·집단동원) vs 동결 43/31/17/9, 같은 페르소나 49/97. 라이브의 F4는 브랜드·상업형이고 동결의 F4는 미디어노출형이라 같은 페르소나명이 다른 raw 내용을 담는다.
