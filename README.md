...
> **핵심 요약판.**  제출 보고서(`(분석보고서)…최종.pdf`, `(요약보고서)…최종.pdf`)의 최종 결과·대표 차트·모델 설명을
...
> **팬덤의 영향력은 "얼마나 큰가" 하나로 줄어들지 않는다.** 안으로 뭉치는 힘(팬충성도)과 밖으로 번지는 힘(파급효과)은 같은 근거 예산을 놓고
> 트레이드오프하고, 영향력이 **어느 경로(F1∼F5)로 나가는지**가 팬덤의 유형을 가른다. 이 저장소는 그 구조를 100개 팬덤 · 10,020건의 근거 문장에서
> 재현 가능한 숫자로 보여 준다.

| 이 분석이 답하는 질문 | 답 |
|---|---|
| **무엇을 재는가** | 팬충성도(안으로 뭉치는 힘) · 파급효과(밖으로 번지는 힘) · 팬 요인 다양성(영향력의 구조)의 3축, 그리고 광고·미디어·결속·지역·언어 등 보조지표 7종 |
| **어떻게 재는가** | 근거 문장을 LDA 토픽(K=10) → 메타팩터(M=5) → 파급경로(F1∼F5) → 페르소나(4유형)로 해석하고, 문장별 EvidenceScore를 합산해 점수화. 재군집화는 실루엣 게이트(0.267)를 넘을 때만 채택 |
| **무엇을 발견했는가** | ① 충성도와 파급효과는 활동량을 통제하면 부호가 반전되는 트레이드오프 관계 ② 실현된 페르소나 4유형 모두 F3(현장경제)를 공유 — 콘서트 참여가 유형 분화의 출발점 ③ 근거를 아무리 늘려도 군집 안정성은 좋아지지 않아 해석 계층을 동결 스냅샷에 고정 |

| 팬덤 | 근거 문장 | 언어권 | 해석 계층 | 페르소나 | 보조지표 | 게이트 기각 | 저장소 검증 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **100** | **10,020** | **14** | K=10 → M=5 → F1∼F5 | **4유형** (43/31/17/9) | **7종** | **28회 연속** | 101/101 · 238/238 |

**바로 보기** — 회전되는 [3D 포지셔닝 맵](3D_포지셔닝맵_국내100팬덤.html) · 페르소나가 결정되는 [결정공간](Persona_결정공간.html) · 인용 수치의 근거 파일 [`KEY_FINDINGS.md`](v7_final_10020/docs/KEY_FINDINGS.md) · 산식을 코퍼스에서 직접 계산해 보는 [지표 산정 노트북](v7_final_10020/index_methodology/index_calculation_v7.ipynb)
