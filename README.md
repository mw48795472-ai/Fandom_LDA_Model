...
**해석은 어떻게 만들어지는가.** 문장을 LDA로 열 개의 토픽으로 묶고, 토픽 간 단어 분포가 비슷한 것끼리 다시 다섯 개 메타팩터로 묶은 뒤,
각 메타팩터가 팬덤 활동을 어디로 전이시키는지에 따라 F1(팬덤 결속)부터 F5(대중·글로벌 확산)까지 경로 이름을 붙인다. 팬덤마다 비중이 큰
경로 두 개를 고르면 페르소나가 정해진다. 이 해석 계층은 코퍼스가 바뀔 때마다 다시 만들지 않고, 군집 품질(실루엣)이 검증된 시점의 스냅샷에
고정해 두었다. 이후 스물여덟 번의 재군집화가 모두 그 기준을 넘지 못했기 때문이다.
...
군집 안정성은 좋아지지 않았다. 리서치를 더 하면 통계가 안정된다는 가정은 이 데이터에서 성립하지 않았고, 그래서 해석 계층과 라이브 지표를
분리하는 이원 구조를 택했다.
...
- 해석 계층(K→F→페르소나, 전체 순위표)은 v7-40 스냅샷(7,350건)에 고정되어 있고, 로스터 교체 3건(BE'O→빈지노 · 한로로→몬스타엑스 · pH-1→투어스)을 포함한 라이브 코퍼스는 보조지표에만 반영된다. 동결 코퍼스 원본은 남아 있지 않아 `data/v7_final/frozen_snapshot_v7_40/`의 근사 복원본(97개 팬덤은 문장 집합까지 동일, 교체된 3개 팬덤은 r22 시점까지, 7,326/7,350건)으로 대신한다. 그 복원본과 재구성 토크나이저로 동결 모델을 다시 적합해도 실루엣 0.267은 재현되지 않으므로(0.07; 시드 10개 × K∈{8,10} × 코퍼스 2개 40회 적합에서도 최대 0.192, 0.267 도달 0회), 해석 계층의 수치는 재현되지 않은 스냅샷 값으로 읽어야 한다. 게이트를 '단일 실루엣 임계'에서 '시드 분포·안정성 합의'로 바꾸는 개편안(v2)과 측정된 적합 4건의 판정표는 `v7_final_10020/silhouette_gate_policy/GATE_POLICY_V2_PROPOSAL_V7.md`에 있다(v2로는 라이브 K=8만 통과; 제안이며 본문은 바꾸지 않음). 그 통과 모델의 해석 계층을 같은 규칙으로 만들어 동결과 대조한 병행 트랙은 `v7_final_10020/analysis/persona_decision_space/live_interpretive_layer/`에 있다: 상위 15 겹침 12/15, 공통 97개 순위 ρ=0.83, 페르소나 라이브 62/17/12/9(글로벌투어·원정소비·현장상업·집단동원) vs 동결 43/31/17/9, 같은 페르소나 49/97. 라이브의 F4는 브랜드·상업형이고 동결의 F4는 미디어노출형이라 같은 페르소나명이 다른 raw 내용을 담는다.
