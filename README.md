...
> 한 페이지로 압축했다. 라운드별 진행 이력은 생략하고, 저장소의 모든 수치는 `verify_v7_final_consistency.py`(110/110)와 R 교차검증(238/238)으로 파일에서 재현된다.
...
| 핵심 모델 | **LDA 토픽(K=8) → 메타팩터(M=5) → 파급경로(F1∼F5) → 페르소나(4유형)** 4단계 해석 계층. 해석 계층은 **r73 라운드(투어스 영문 근거 86건을 한국어로 재작성한 코퍼스 10,020건)의 재적합(K=8, M=5 고정, 실루엣 0.092)** 이며, 동결 스냅샷(v7-40, 7,350건, K=10, 실루엣 0.267)은 재현되지 않는 역사 값으로 보관. r73은 게이트 v2의 G1(시드 중앙값 0.081 < 현직 0.097)을 넘지 못했지만 텍스트 품질 수정 라운드(문서 0.9% 변경)로 저자 결정에 따라 채택(정책 v2.1) |
...
경로 두 개를 고르면 페르소나가 정해진다. 이 해석 계층은 처음에는 군집 품질(실루엣)이 검증된 시점의 동결 스냅샷(v7-40)에 고정돼 있었고 이후 스물여덟 번의 재군집화가 모두 그 기준(0.267)을 넘지 못했다. 그러나 그 기준선은 저장소 재료로 어느 시드로도 재현되지 않았고(40회 적합 중 0회) 실루엣 자체가 시드에 따라 두 배 가까이 움직여, 게이트를 '단일 실루엣 임계'에서 '시드 분포·토픽 안정성 합의'(v2)로 바꿨다. v2를 통과한 최종 라이브 코퍼스(r72)의 K=8 참고 재적합을 먼저 해석 계층으로 삼았고, 그 계층에서 투어스(TWS)가 영문 근거 때문에 100위·PCA 아웃라이어로 나타나는 수집 언어 문제를 찾아 근거 86건을 한국어로 다시 쓴 r73 라운드의 재적합이 지금의 해석 계층이다. r73은 v2의 G1을 넘지 못했으나(시드 중앙값 0.081 < 0.097) 코퍼스 차이가 0.9%라 시드 변동 폭 안이므로 텍스트 품질 수정 라운드로 채택했다.
...
군집 안정성은 좋아지지 않았다. 리서치를 더 하면 통계가 안정된다는 가정은 이 데이터에서 성립하지 않았고, 그래서 처음에는 해석 계층과 라이브 지표를
분리하는 이원 구조를 택했다가, 기준선의 재현 불가와 시드 변동을 확인한 뒤 게이트 v2로 해석 계층을 라이브로 옮겼다(현재 r73).
...
3. **리서치를 늘려도 군집 안정성은 좋아지지 않았다.** 코퍼스가 1,781건→9,680건(+444%)으로 자라는 동안 K→M 재군집화의 실루엣은 0.058∼0.148을 등락했고 기준선 0.267에 한 번도 닿지 못했다(게이트 구간 20회 실측, 최종 보고서 시점 누적 **28회 연속 기각**). 그래서 최종 보고서 시점에는 해석 계층을 v7-40 스냅샷에 고정하고, 라이브 코퍼스는 보조지표에만 반영하는 **이원 구조**를 택했다.
...
|---|---|---|---|
| r73 (해석 계층) | 10,020건 (투어스 86건 한국어 재작성) | K=8→M=5 고정(실루엣 0.092, 시드 중앙값 0.081), F1∼F5 비중, 페르소나 89/8/2/1(현장상업·글로벌투어·집단동원·산업확장), 전체 순위표(투어스 32위) | `data/v7_final/r73_tws_ko/`, `analysis/persona_decision_space/live_interpretive_layer/` |
| 라이브 코퍼스 r72 (지표·통계·3D 맵) | 10,020건 | 팬충성도·파급효과 점수(평균 0.371/0.266; 투어스 외 99개 팬덤은 r73과 동일), 4구획 24/17/10/49, 통계 검정, 보조지표 7종, 3D 맵, 참고 재적합 K=8→M=5(실루엣 0.046, 시드 중앙값 0.102; 게이트 v2 통과) | `fandoms_v3_100.json`, `fandom_scores_live_reference_v7.json`, `chart3d_payload_live_reference_v7.json`, `analysis/persona_decision_space/live_interpretive_layer_r72/` |
| 동결 스냅샷 v7-40 (역사 값, 보관) | 7,350건 | K=10→M=5(실루엣 0.267 — 재현되지 않음), F1∼F5 비중, 페르소나 43/31/17/9, 최종 보고서 PDF의 순위표 | `fandom_scores_v6.json`, `fan_persona_v7.json`, `lda_v6_diagnostics_frozen_v7_40.json` |
...
| 검정 | 결과 | 해석 |
|---|---|---|
| 정규성(Shapiro-Wilk) | 두 점수 모두 p<.05 | 정규분포 아님 → 순위 검정 병행. 회귀 두 개를 Huber·중위수 분위회귀·영향점 5개 제외 OLS로 다시 적합해도 계수 부호·유의성이 라이브·동결 모두 일치(`Statistics/ROBUST_REGRESSION_V7.md`) |
...
## 4. 팬충성도 + 파급효과 상위 15개 팬덤 (r73 해석 계층)
...
| 12 | 아이유 | 0.59 | 0.42 | 미디어노출형 | 핵심전략형 | 15 |
| 13 | EXO | 0.56 | 0.39 | 현장경제형 | 핵심전략형 | 21 |
| 14 | NewJeans | 0.39 | 0.55 | 현장경제형 | 핵심전략형 | 11 |
| 15 | 싸이 | 0.35 | 0.56 | 현장경제형 | 외부견인형 | 16 |

점수는 EvidenceScore 합의 min-max라 모델과 무관하며, r73에서 바뀐 것은 투어스(TWS) 하나다(100위 → 32위, 팬충성도 0.03→0.34, 파급효과 0.07→0.37; 다른 99개 팬덤은 r72와 동일). 동결 스냅샷 순위표(최종 보고서 PDF 4절)와 상위 15 겹침 12/15, 공통 97개 팬덤 순위 Spearman 0.83. 전체 100개 순위는 `v7_final_10020/analysis/persona_decision_space/live_interpretive_layer/live_full_ranking_v7.csv`. 점수가 팬덤별 불릿 건수 구조에 좌우된다는 점(8절, L10·L12)은 이 표에도 그대로 적용된다.
...
| 계층 | 답하는 질문 | 정의 | 파일 |
|---|---|---|---|
| K (Topic) | 무엇을 하는가 | LDA 토픽 8개 (단독콘서트매진형·해외보도수상형·예능출연활동형·브랜드앰버서더형·음원차트기록형·팬클럽기부투어형·현지매체보도형·글로벌음반판매형) | `live_interpretive_layer/live_k_to_f_v7.csv` |
...
| Persona | 어떤 방식으로 작동하는가 | 상위 2개 F 조합 10가지 중 실현 4유형: **현장상업형 89**(F3+F4) · **글로벌투어형 8**(F3+F5) · **집단동원형 2**(F1+F3) · **산업확장형 1**(F4+F5) | `live_interpretive_layer/live_persona_v7.json` |
| Loyalty·Spillover | 얼마나 강한가 | 점수를 F별로 분해한 Factor-specific Impact = 비중 × 점수 | 같은 파일 |

| 토픽 | 이름 | 상위 10단어 | F (파급경로) |
|---|---|---|---|
| T0 | 단독콘서트매진형(콘서트·공연·서울·단독) | 콘서트, 공연, 서울, 단독, 티켓, 2025년, 2026년, 매진, 데뷔, 투어 | **F3** 현장경제 경로 |
| T1 | 해외보도수상형(일본·수상·기사·홍보대사) | 일본, 수상, 기사, 홍보대사, 소개, 보도, 확인, 위촉, 매체, 아랍어 | **F3** 현장경제 경로 |
| T2 | 예능출연활동형(출연·예능·활동·무대) | 출연, 예능, 활동, 2026년, 함께, 멤버, 출연해, 무대, mbc, 콘텐츠 | **F4** 산업전이 경로 |
| T3 | 브랜드앰버서더형(브랜드·모델·광고·앰버서더) | 브랜드, 모델, 광고, 앰버서더, 발탁, ost, 캠페인, 글로벌, 드라마, 2025년 | **F4** 산업전이 경로 |
| T4 | 음원차트기록형(기록·1위·앨범·발매) | 기록, 1위, 앨범, 발매, 데뷔, 차트, 싱글, 빌보드, 판매, 이상 | **F5** 대중·글로벌 확산 경로 |
| T5 | 팬클럽기부투어형(팬클럽·공식·tour·기부) | 팬클럽, 공식, tour, 기부, concert, fan, 팬덤, world, japan, 원을 | **F1** 팬덤결속 경로 |
| T6 | 현지매체보도형(보도·매체·공연·태국) | 보도, 매체, 공연, 팬들, 태국, 2026년, 대만, 소식, 콘서트, 데뷔 | **F3** 현장경제 경로 |
| T7 | 글로벌음반판매형(million·album·music·group) | million, album, music, group, pop, chart, japan, sold, korea, debut | **F2** 직접소비 경로 |

r73 모델은 '출연·예능' 토픽(T2)과 '브랜드·광고' 토픽(T3)을 함께 미디어노출형(F4 산업전이)으로 묶는다. 그래서 대부분의 팬덤이 F3 현장경제 + F4 산업전이 조합인 현장상업형이 되고, 페르소나 분포가 동결(43/31/17/9)·r72 참고 재적합(글로벌투어 62·원정소비 17·현장상업 12·집단동원 9)과 크게 다르다 — 동결 대비 같은 페르소나 37/97(이동: 글로벌투어형→현장상업형 33, 현장상업형→현장상업형 30, 원정소비형→현장상업형 16, 집단동원형→현장상업형 8), r72 대비 같은 페르소나 19/100. 코퍼스가 0.9%만 달라져도 토픽→F 묶음이 바뀌는 것은 L7(시드 민감성)이 보여준 성질이며, 페르소나 유형의 **개수와 이름표는 안정적이지 않고 F3(현장경제)이 1위 경로라는 구조만 안정적**이라고 읽어야 한다(1위 경로 F3: r73 81/100, 동결 96/100). 이동표는 `live_interpretive_layer/persona_migration_v7.csv`, r72 계층은 `live_interpretive_layer_r72/`.

<p align="center"><img src="assets/readme/fig05_persona_map.png" alt="Fan Persona Map" width="640"></p>

*그림 5. Fan Persona Map — 팬충성도 × 파급효과 평면의 페르소나 분포(현장상업형 89 · 글로벌투어형 8 · 집단동원형 2 · 산업확장형 1). 점선은 표본 평균(0.371/0.266). r72 판은 `fig05_persona_map_r72.png`, 동결 판은 `fig05_persona_map_frozen_v7_40.png`.*

![경로별(F1∼F5) 점수 분해](assets/readme/fig06_factor_specific_impact.png)

*그림 6. 경로별(F1∼F5) 팬충성도·파급효과 분해 — BTS·임영웅·리센느와 합산 상위 17개 팬덤. 세 팬덤 모두 1위 경로는 F3(현장경제)이고 2위는 F4(산업전이)다. F5(글로벌 확산) 파급효과 기여는 BTS 0.125 · 임영웅 0.115 · 리센느 0.048.*

<p align="center"><img src="assets/readme/fig07b_persona_pca.png" alt="Persona PCA" width="760"></p>

*그림 7. 페르소나를 결정하는 F1∼F5 비중 공간의 PCA(PC1 59.0%, PC2 22.5%; 오른쪽 패널은 F별 loading 방향). r72 계층에서 홀로 떨어져 있던 투어스(TWS)는 영문 근거 86건을 한국어로 재작성한 뒤 글로벌투어형(F3 0.49·F5 0.22)으로 중심 근처(중심 거리 30위/100)에 놓인다(`fig07b_persona_pca_r72.png`와 비교). 라이브 재적합의 φ·병합 높이는 저장되지 않아 덴드로그램은 없고 동결 K=10 덴드로그램은 `fig07a_persona_dendrogram_frozen_v7_40.png`. 생성: `v7_final_10020/charts/build_live_layer_figures_v7.py`.*
...
| 국내 지역 지수 (사실상 지역 활동 지수: 활동 84% · 연고 6%) | 17개 시/도 밀착도 | 지역명 텍스트마이닝 | 지역 언급 1,226건, 검출 469, 99/100 팬덤. 서울 편중 뚜렷, 다양성 1위 임영웅(12지역, 0.71) |
| 멤버 집중도 MCI | 그룹 내 멤버 쏠림 | Σ(멤버 점유율)² (허핀달 방식, 하한 1/멤버수) | 45개 그룹. MCI∼멤버 수 r=−0.728, 멤버 수 통제 후 outcome 설명력 R²<0.05 |
| 세계 언어 지수 (= 출처 언어권 다양성; 본문 언어 아님) | 해외 확산 | Coverage의 언어 분포 재구성 | 해외언어 4,469건(44.6%), 100/100 팬덤. 영어 2,195(49.1%) · 일본어 · 중국어 순. 본문 기준으로 다시 판정하면 84.5%가 한국어(수집 시 한국어 요약)라 지수는 출처 도메인 언어권으로 읽는다(`analysis/worldwide_language_index/BODY_LANGUAGE_V7.md`) |
...
원시 MCI가 가장 잘 설명하는 outcome은 팬충성도(r=−0.385, R²=0.148)지만, 멤버 수를 뺀 MCI_excess로는 어떤 outcome도 유의하지 않다(최대 R²=0.046). MCI는 집중도보다 멤버 수를 대리하는 지표에 가깝다. 이 수치는 최종 MCI × 동결 점수라 시점이 섞여 있는데, 라이브 점수(10,020건)로 시점을 맞춰도 같다: 원시 MCI∼팬충성도 r=−0.390(p=0.008), MCI_excess 최대 R²=0.028, 정규화 HHI 최대 R²=0.035(`v7_final_10020/analysis/group_member_index/MCI_SAME_PERIOD_V7.md`).
...
**K 토픽 · F 메타팩터 워드클라우드 (동결 스냅샷 v7-40 — 역사 값; 라이브 K=8 토픽의 상위어는 5절 표)**
...
- 해석 계층(K→F→페르소나, 전체 순위표)은 게이트 v2 채택으로 라이브 코퍼스(10,020건, K=8)로 옮겼고, 이어 투어스 영문 근거 86건을 한국어로 재작성한 r73 라운드의 재적합으로 갱신했다. 최종 보고서 PDF의 해석 계층은 v7-40 스냅샷(7,350건, K=10)이다. 두 계층은 상위 15 겹침 12/15, 같은 페르소나 37/97로 다르며 PDF와 이 README의 4·5절 수치는 일치하지 않는다. r73은 게이트 v2의 G1(시드 중앙값 0.081 < 현직 0.097)을 넘지 못했지만 코퍼스 변경이 0.9%라 시드 변동 폭 안이고, 텍스트 품질 수정 라운드는 게이트 대상이 아니라 현직 재측정으로 취급한다는 저자 결정(정책 v2.1, `data/v7_rounds/round_log_r73.json`)으로 채택했다. 그 대가로 페르소나 분포가 r72 참고 재적합(62/17/12/9)에서 r73(89/8/2/1)로 크게 움직였다 — 페르소나 유형의 개수·이름표가 코퍼스 0.9% 변경에도 흔들린다는 것이 이 데이터의 성질이다. 로스터 교체 3건(BE'O→빈지노 · 한로로→몬스타엑스 · pH-1→투어스)은 라이브에만 있다. 동결 코퍼스 원본은 남아 있지 않아 `data/v7_final/frozen_snapshot_v7_40/`의 근사 복원본(97개 팬덤은 문장 집합까지 동일, 교체된 3개 팬덤은 r22 시점까지, 7,326/7,350건)으로 대신한다. 그 복원본과 재구성 토크나이저로 동결 모델을 다시 적합해도 실루엣 0.267은 재현되지 않으므로(0.07; 시드 10개 × K∈{8,10} × 코퍼스 2개 40회 적합에서도 최대 0.192, 0.267 도달 0회), 해석 계층의 수치는 재현되지 않은 스냅샷 값으로 읽어야 한다. 게이트를 '단일 실루엣 임계'에서 '시드 분포·안정성 합의'로 바꾸는 개편안(v2)과 측정된 적합 4건의 판정표는 `v7_final_10020/silhouette_gate_policy/GATE_POLICY_V2_PROPOSAL_V7.md`에 있다(v2로는 라이브 K=8만 통과; 2026-09-23 채택). 그 통과 모델(r72)의 해석 계층은 `v7_final_10020/analysis/persona_decision_space/live_interpretive_layer_r72/`에, 현재 채택된 r73 계층은 `live_interpretive_layer/`에 있다(4·5절). r72 계층에서 투어스(TWS)가 100위·F2 비중 0.74의 PCA 아웃라이어였던 원인은 r63 교체 리서치가 근거 90건 중 86건을 영문으로 적은 수집 언어 문제였다(영문 본문 비중과 F2 비중의 상관 r=0.97, 영문 문장의 EvidenceScore 평균 1.10 vs 한국어 1.78). r73은 그 86건을 같은 사실·URL로 한국어 요약으로 다시 쓴 라운드다(`data/v7_final/r73_tws_ko/`).
- 판별타당도(|r|<0.5)와 Z축 독립성은 스냅샷에 따라 결론이 뒤집혔다. 단일 스냅샷만으로 통계적 결론을 확정하기 어렵다는 근거다. 팬덤 부트스트랩(B=2,000)으로 재면 r의 95% CI는 라이브 [0.25, 0.67]·동결 [0.46, 0.77]로 둘 다 0.5를 포함하고, 두 스냅샷의 r 차이 0.126은 유의하다(CI [0.03, 0.24]). 그 차이의 대부분은 팬덤별 불릿 건수 구조에서 온다: 라이브 문장을 동결 시점 건수만큼 무작위로 줄이기만 해도 r이 0.62로 동결 값 쪽에 가고, 점수 자체가 불릿 건수와 r≈0.90∼0.96으로 묶여 있다(`Statistics/BOOTSTRAP_CI_V7.md`). 건수를 걷어낸 밀도 정의(문장당 평균 EvidenceScore)로 보면 r은 라이브 0.43·동결 0.40으로 두 스냅샷 모두 기준을 만족하고 반전이 사라지지만, 상위 15 순위는 원 지수와 5/15만 겹친다(`v7_final_10020/index_methodology/EXPOSURE_ADJUSTMENT_V7.md`). 원 지수는 규모, 밀도는 근거 밀도라는 다른 질문에 답한다.
- 보조지표는 사전에 없는 표현(다큐멘터리·라디오, 사전 밖 브랜드명)을 포착하지 못하고, MCI는 멤버 수가 적을수록 구조적으로 높다. 누락의 크기를 가늠하면, 사전 밖 유사 표현이 든 미매칭 불릿이 광고 284건(미매칭의 3.3%), 미디어 214건(2.4%: 라디오·다큐·뮤지컬·위버스 등), 지역 93건(1.0%)이고 사람 검수용 표본 200건씩을 `v7_final_10020/analysis/unmatched_audit/`에 뒀다. 그 표본을 모델이 판독한(사람 검수 아님) 재현율 추정은 광고 0.81(95% CI 0.68∼0.90; 놓친 것은 대부분 영문 'brand ambassador'류), 미디어 0.57(0.46∼0.68; 런닝맨·아는 형님·쇼미더머니 같은 프로그램명·라디오·서바이벌), 지역 0.78(0.62∼0.88; 영문 지명, 봉화·철원·해남 같은 사전 밖 군 단위)이며 사전 추가 후보는 `analysis/dictionaries/dictionary_candidates_from_audit_v7.csv`에 있다.
...
```bash
python verify_v7_final_consistency.py                                        # 보고서 수치 ↔ data/v7_final 110/110
...
**저장소에 없는 것**: 동결 스냅샷의 φ·코사인 거리 행렬·학습 모델(원본 데이터 묶음에도 파일로 없었고 병합 높이 9개만 `Persona_결정공간.html`에 남아 있음). 동결 근사 코퍼스와 재구성 토크나이저로 K=10을 다시 적합해 봤으나(`v7_final_10020/analysis/persona_decision_space/topic_phi_cosine/frozen_v7_40/`) **동결 스냅샷은 재현되지 않았다** — M=5 실루엣 0.07(동결 0.267), 토픽 10개 중 1개만 상위 10단어 8개 이상 겹침, 팬덤별 F 비중은 두 F에서만 r≈0.75~0.82. 원 저자의 동기화 코드 재현 시도(0/10 토픽)와 같은 결론이다. 따라서 해석 계층의 K=10·M=5·0.267·페르소나 43/31/17/9는 '저장된 스냅샷 값이며 재구성 입력으로는 재현되지 않는다'는 단서를 달고 인용한다. 같은 방법을 최종 코퍼스에 적용한 φ·코사인 거리 산출물과 학습 모델 pickle·단어 사전·manifest는 `v7_final_10020/analysis/persona_decision_space/topic_phi_cosine/`에 있다. 0.267이 시드 운인지도 확인했다: 시드 0∼9 × K∈{8,10} × 코퍼스 2개(라이브·동결 근사) 40회 적합에서 M=5 실루엣은 0.040∼0.175, M을 자유롭게 골라도 최대 0.192로 0.267에 닿는 시드가 없고, 시드 간 토픽 상위 10단어 Jaccard 중앙값은 0.39∼0.40이다(`…/topic_phi_cosine/seed_stability/SEED_STABILITY_V7.md`). 단일 시드로 확정한 K·M·토픽 라벨은 그 변동 폭 안의 한 값이다.
...
verify_v7_final_consistency.py              최종 수치 ↔ 파일 정합성 검증 (110/110)
...
data/
  v7_final/            최종 코퍼스 10,020건 + 최종 산출물 + 동결 스냅샷 산출물 (폴더 README)
    ...
    r73_tws_ko/          r73 라운드: 투어스 86건 한국어 재작성 코퍼스(10,020건) + 재적합 점수·진단 + 검토용 재작성 CSV (폴더 README)
  v7_rounds/           v6 단계 로그 3개 + v7 병합·교체 로그 전량 (r72 10,020건, r73은 투어스 재작성·순증 0)
