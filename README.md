...
| 토픽 | 이름 | F |
|---|---|---|
| T0 | 브랜드앰버서더형(브랜드·광고·모델·앰버서더) | F4 산업전이 경로 |
| T1 | 음원차트기록형(기록·1위·앨범·발매) | F5 대중·글로벌 확산 경로 |
| T2 | 단독콘서트투어형(콘서트·공연·단독·투어) | F3 현장경제 경로 |
| T3 | 해외투어음반형(fan·japan·tour·album) | F2 직접소비 경로 |
| T4 | 페스티벌무대형(무대·축제·페스티벌·대학축제) | F3 현장경제 경로 |
| T5 | 예능영화출연형(출연·예능·영화·일본) | F3 현장경제 경로 |
| T6 | 팬클럽공식활동형(공식·팬클럽·홍보대사·팬덤) | F3 현장경제 경로 |
| T7 | 드라마OST기부형(ost·드라마·기부·수상) | F1 팬덤결속 경로 |
...
| K→F 배정표 | PCA 투영 (페르소나 결정공간) |
|---|---|
| ![k to f](assets/readme/fig07a_persona_k_to_f.png) | ![pca](assets/readme/fig07b_persona_pca.png) |

*그림 7. 왼쪽은 라이브 K=8 토픽의 F 배정표(라이브 참고 재적합의 φ·병합 높이는 저장되지 않아 덴드로그램 대신; 동결 K=10 덴드로그램은 `fig07a_persona_dendrogram_frozen_v7_40.png`), 오른쪽은 페르소나를 결정하는 F1∼F5 비중 공간의 PCA(PC1 56.6%, PC2 23.1%). 생성: `v7_final_10020/charts/build_live_layer_figures_v7.py`. 동결 계층의 재현 노트북은 `v7_final_10020/analysis/persona_decision_space/`(K→F 10/10, 페르소나 100/100).*
...
- 해석 계층(K→F→페르소나, 전체 순위표)은 게이트 v2 채택으로 라이브 코퍼스(10,020건, K=8)로 옮겼고, 최종 보고서 PDF의 해석 계층은 v7-40 스냅샷(7,350건, K=10)이다. 두 계층은 상위 15 겹침 12/15, 같은 페르소나 49/97로 다르며 PDF와 이 README의 4·5절 수치는 일치하지 않는다. 로스터 교체 3건(BE'O→빈지노 · 한로로→몬스타엑스 · pH-1→투어스)은 라이브에만 있다. 동결 코퍼스 원본은 남아 있지 않아 `data/v7_final/frozen_snapshot_v7_40/`의 근사 복원본(97개 팬덤은 문장 집합까지 동일, 교체된 3개 팬덤은 r22 시점까지, 7,326/7,350건)으로 대신한다. 그 복원본과 재구성 토크나이저로 동결 모델을 다시 적합해도 실루엣 0.267은 재현되지 않으므로(0.07; 시드 10개 × K∈{8,10} × 코퍼스 2개 40회 적합에서도 최대 0.192, 0.267 도달 0회), 해석 계층의 수치는 재현되지 않은 스냅샷 값으로 읽어야 한다. 게이트를 '단일 실루엣 임계'에서 '시드 분포·안정성 합의'로 바꾸는 개편안(v2)과 측정된 적합 4건의 판정표는 `v7_final_10020/silhouette_gate_policy/GATE_POLICY_V2_PROPOSAL_V7.md`에 있다(v2로는 라이브 K=8만 통과; 2026-09-23 채택). 그 통과 모델의 해석 계층을 같은 규칙으로 만든 것이 4·5절이며 동결과의 대조는 `v7_final_10020/analysis/persona_decision_space/live_interpretive_layer/`에 있다: 상위 15 겹침 12/15, 공통 97개 순위 ρ=0.83, 페르소나 라이브 62/17/12/9(글로벌투어·원정소비·현장상업·집단동원) vs 동결 43/31/17/9, 같은 페르소나 49/97. 라이브의 F4는 브랜드·상업형이고 동결의 F4는 미디어노출형이라 같은 페르소나명이 다른 raw 내용을 담는다.
