...
# 수 없으므로, 이 CSV는 "이 세션이 가진 실제 코퍼스(v6 r22, 5,612건)에 새로
# 적용한 병행 산출물"로 취급해야 한다.
#
# 산출: data/v6_r22_snapshot/csv/bullet_token_frequency_v6_r22.csv
...
def main():
    ...
    print(f"[집계] 불릿 수: {bullet_count}건 (기대: 5,612)")
    assert bullet_count == 5612
    ...
    print(f"[검증] 비중(%) 합계: {share_sum:.2f}% (기대: 100.00% 근방, 반올림 오차 허용)")
