...
# 코퍼스(v6 r22 스냅샷)에 적용한 결과를 재검증한다.
#
# 문서: docs/JIEBA_AND_THAI_TOKENIZER_ENGINE_SPEC.md 참고.
#
# 이 스크립트가 하는 것:
...
#   4) scripts/tokenizer/build_bullet_token_frequency_csv.py가 이미 산출한
#      data/v6_r22_snapshot/csv/bullet_token_frequency_v6_r22.csv의 중국어·
...
FREQ_CSV = BASE / "data" / "v6_r22_snapshot" / "csv" / "bullet_token_frequency_v6_r22.csv"
...
def reverify_against_frequency_csv():
    section("5. data/.../bullet_token_frequency_v6_r22.csv의 중국어·태국어 버킷 재검증")
    print("주의: CSV의 '문자권' 컬럼은 어느 하위 토크나이저가 그 토큰을 만들었는지가")
    print("아니라, 최종 토큰 표면형 자체에 어떤 문자가 있는지로 사후 분류한 것이다")
    print("(build_bullet_token_frequency_csv.py의 script_of() 참고) — 그래서 '일반")
