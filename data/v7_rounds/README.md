...
| `merge_log_r1.json` ∼ `merge_log_r72.json` (60개) | 리서치 병합 로그. r1∼r22는 r22 스냅샷의 성장 이력 CSV(archive)와 일치. r36∼r59는 `before_total_bullets`/`after_total_bullets` 키, 나머지는 `before_total`/`after_total` 키. r58은 `round_type=roster_swap`(BE'O→빈지노). 마지막 r72가 9,939 → **10,020건**(최종 라이브 코퍼스) |
