...
자동 명명(재현 가능, 사람이 임의로 붙인 이름 아님). 아래 표의 F코드 배정은 `Persona_결정공간.html`에 내장된
...
- **4분면 독립성**: 2×2 카이제곱(Yates 보정) χ²=8.3439, dof=1, p=0.003869 → 두 축 독립 가정 기각, 4구획 구조
  유의미 (라이브 코퍼스 기준. 프로즌 스냅샷 기준은 χ²=10.2273, p=0.0013837). **주의**: 이 두 값은 저장소의
  파일로 재현되지 않는다 — 라이브 4구획표 24/17/10/49는 Yates χ²=16.84(p<.001), 동결 CSV 평균 기준 29/20/13/38은
  χ²=10.30(p=0.0013). 어느 시점·분할표에서 계산됐는지 원본 기록이 없어 보고서 값을 그대로 인용만 한다.
  (3절의 나머지 통계는 라이브 점수로 전부 재현됨)
