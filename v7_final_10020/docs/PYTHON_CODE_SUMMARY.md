...
| 해석 계층 그림 | `v7_final_10020/charts/build_live_layer_figures_v7.py` (글꼴은 `fonts/NotoSansCJKkr-*.otf` 또는 `KFONT_PATH`; `--layer 폴더 --suffix 접미사`로 다른 계층) | `live_interpretive_layer/` JSON·CSV, 3D payload | `assets/readme/fig05_persona_map.png`·`fig06_factor_specific_impact.png`·`fig07b_persona_pca.png`(2패널 PCA: 좌표 + 로딩) (K→F 배정은 README 5절 표) + `live_pca_v7.json` (동결 판은 `*_frozen_v7_40.png`) |
