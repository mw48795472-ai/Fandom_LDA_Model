...
| 분류 | 스크립트 | 입력 | 출력·확인 |
|---|---|---|---|
| 정합성 검증(루트) | `verify_v7_final_consistency.py` | `data/v7_final/*` 전부 | 보고서·KEY_FINDINGS 수치 110개 항목을 파일에서 재계산해 일치/불일치 출력 (110/110; [AD]절 4항목이 동결 코퍼스 근사 복원본 검증, [AF]절 3항목이 r73 라운드·채택 해석 계층 검증) |
...
| 해석 계층 그림 | `v7_final_10020/charts/build_live_layer_figures_v7.py` (`--layer 폴더 --suffix _r72`로 다른 계층; 글꼴은 `fonts/NotoSansCJKkr-*.otf` 또는 `KFONT_PATH`) | `live_interpretive_layer/` JSON·CSV, 3D payload | `assets/readme/fig05_persona_map.png`·`fig06_factor_specific_impact.png`·`fig07b_persona_pca.png`(2패널 PCA: 좌표 + 로딩) + `live_pca_v7.json` (r72 판은 `*_r72.png`, 동결 판은 `*_frozen_v7_40.png`) |
| 라이브 해석 계층 | `v7_final_10020/analysis/persona_decision_space/build_live_interpretive_layer_v7.py` (기본 r73; `--r72`로 r72 계층, `--scores/--diag/--out/--label`로 임의 계층) | 점수 JSON(factor_share; 기본 `r73_tws_ko/fandom_scores_r73.json`), 재적합 진단 JSON(topic_to_factor; 기본 `lda_v6_diagnostics_r73.json`), `fan_persona_v7.json`(정의표), `factor_pathway_map_v7.json`, 3D payload(표본 평균) | `live_interpretive_layer/`(r73: 4구획 24/17/11/48, 페르소나 89/8/2/1)·`live_interpretive_layer_r72/`(4구획 24/17/10/49) — 순위표·K→F·페르소나 JSON·동결→라이브 이동표·요약 |
...
| 시드 안정성 | `v7_final_10020/analysis/persona_decision_space/topic_phi_cosine/build_seed_stability_v7.py` (`--corpus 이름=경로 --k 8,10 --out 폴더`로 다른 코퍼스) | 라이브 10,020건·동결 근사 7,326건(기본) 또는 r73 코퍼스, 재구성 토크나이저 마커 절 | `seed_stability/`·`seed_stability_r73/` — 코퍼스×K×시드 0∼9 의 M=4∼8 실루엣 CSV, 시드 쌍별 토픽 Jaccard CSV, 요약 JSON, `SEED_STABILITY_V7.md` (약 30분; r73 K=8만은 약 8분) |
...
| 데이터 export | `v7_final_10020/data_export/build_corpus_growth_history_csv.py` | `data/v7_rounds/v6*_merge_log.json` + `merge_log_r*.json`·`swap_log_r*.json`·`round_log_r70.json`·`round_log_r73.json` | `corpus_growth_history_v6_v7_full.csv` — v6 1차∼v7 r73(10,020건, 마지막 행은 순증 0의 재작성 라운드 `kind=rewrite`) 성장 이력, 체인 불연속을 `chain_gap`에 기록, 타임라인 CSV와 대조 |
...
`--force-m N`은 M을 고정하고 진단 JSON에 `m_grid_silhouette`(M별 실루엣)와 `m_forced`를 남긴다 — r73 재적합(`data/v7_final/r73_tws_ko/lda_v6_diagnostics_r73.json`, M=5 고정)이 이 옵션으로 만들어졌다(자동 선택은 M=6).
