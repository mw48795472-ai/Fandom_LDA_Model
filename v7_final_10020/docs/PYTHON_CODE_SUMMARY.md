...
| 지표 산정 노트북 | `v7_final_10020/index_methodology/build_index_calculation_notebook.py` → `index_calculation_v7.ipynb`(+`.html`) | 코퍼스 10,020건, 라이브 점수 JSON(coverage_detail·factor_share), 3D payload | 식(1)∼(7)로 100개 팬덤 지표를 직접 산정 → `index_calculation_v7_result.csv`(100×17), 최종 산출 파일과 8개 항목 0/100 불일치 |
