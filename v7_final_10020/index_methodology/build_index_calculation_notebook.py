...
실행: python v7_final_10020/index_methodology/build_index_calculation_notebook.py            # 생성 + 실행 (+ Spyder용 index_calculation_v7.py)
...
def find_repo_root():
    starts = [Path.cwd()]
    if "__file__" in globals():                       # .py 로 실행할 때는 파일 위치에서도 찾는다
...
OUT_PY = HERE / "index_calculation_v7.py"   # Spyder/VS Code 용 셀 스크립트 (# %% 구분) — 노트북과 같은 내용


def write_spyder_script(cells, path):
    """노트북 셀을 Spyder가 여는 .py 로 옮긴다: 마크다운은 주석, 코드는 `# %%` 셀."""
    parts = ["# -*- coding: utf-8 -*-",
             "# index_calculation_v7.ipynb 와 같은 내용의 Spyder 셀 스크립트. 저장소 안 어느 폴더에서 실행해도 된다.",
             "# Spyder: 파일을 연 뒤 F5(전체 실행) 또는 Ctrl+Enter(현재 셀 실행). 표는 IPython 콘솔에 출력된다.",
             "try:\n    display\nexcept NameError:  # IPython 밖에서 실행할 때\n    display = print", ""]
    for c in cells:
