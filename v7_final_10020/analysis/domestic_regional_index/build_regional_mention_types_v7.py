...
EXCLUDE = {"진주": [r"진주\s*(목걸이|귀걸이|반지|장식|색|빛|알|층)", r"흑진주", r"진주(같|처럼)"], "화성": [r"화성(인|탐사|에\s*가|착륙|행성)"], "경주": [r"경주(마|로|용|를|에서\s*우승|\s*대회)", r"(자동차|카트|요트|보트|말)\s*경주"],
           "전주": [r"전주(가|부터|만|를|에서부터|\s*부분|\s*멜로디|\s*연주|\s*파트)", r"곡의\s*전주"], "구미": [r"구미(를|가)\s*(당|돋)", r"구미\s*당"], "진도": [r"진도(율|가|를|로|에\s*따|\s*체크|를\s*나가)"],
           "청주": [r"청주(를|한\s*잔|와|·|,)\s*(마|막걸리|소주)"], "대구": [r"대구(탕|찜|살|포|전|매운탕)"], "남해": [r"남해\s*바다", r"남해에서\s*잡"], "김천": [], "포항": [r"포항(제철|스틸러스)"]}
...
AMBIG = ["광주", "진주", "화성", "경주", "전주", "구미", "진도", "남해", "청주", "대구", "김천", "포항", "김해", "거제", "울릉", "성남", "안산", "보령", "아산", "순천", "제주", "울산"]
...
md.append("\n배제 패턴(예: 진도율·경주마·전주가·구미가 당기다·화성 탐사)에 걸리면 일반어, 지명 접미·문맥어(시·군·광역시·에서·공연·콘서트·출신·고향 …)가 붙으면 확인, 둘 다 아니면 불확실이다. 불확실은 원 지수와 엄격 지수 모두에 남긴다. 사람 검수 표본을 뽑을 자리다.\n")
