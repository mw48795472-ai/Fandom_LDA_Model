...
md.append("1. **입력이 원본과 같지 않다.** 코퍼스는 3개 팬덤 24건이 빠져 있고, 토크나이저는 재구성본이라 저빈도 불용어가 다를 수 있다. LDA는 DTM의 작은 차이에도 토픽 경계가 움직인다.\n"
          ...
          "당시 코퍼스 상태(r39 직후)·토크나이저 판·패키지 버전 중 어느 것이 지금과 다른지 확정할 수 없다. 다만 시드 운은 아니다: 같은 근사 코퍼스·재구성 토크나이저로 시드 0∼9를 돌려도 K=10·M=5 실루엣은 0.055∼0.136(seed 0 = 이 문서의 0.07), M을 자유롭게 골라도 최대 0.162로 0.267에 닿는 시드가 없다(`../seed_stability/SEED_STABILITY_V7.md`).\n"
