...
근사 복원본(7,326건)이 있고 라우팅 토크나이저도 재구성본(저장소 루트 run_lda_v6_live_reference_v7.py)이 있으나 동결 모델 재적합은 아직
하지 않았다. 이 스크립트는 저장소에 실제로
...
manifest = {
    ...
    "tokenizer": {"source": "run_lda_v6.py 의 PARTICLES/STOPWORDS/ENGLISH_STOPWORDS/tokenize() 를 ast 로 추출해 그대로 실행", "run_lda_v6_sha256": sha256(REPO / "run_lda_v6.py"),
                  "note": "보고서의 14개 언어 라우팅 토크나이저(재구성본 run_lda_v6_live_reference_v7.py)가 아니라 구 토크나이저이므로 문서 수가 10,018건과 다르다"},
...
md.append("**이 폴더는 동결 스냅샷(v7-40, 7,350건)의 모델이 아니다.** 보고서 해석 계층이 쓰는 K=10·M=5·실루엣 0.267은 7,350건 코퍼스와 14개 언어 라우팅 토크나이저로 얻은 값이다. "
          ...
          "이 폴더는 같은 방법을 저장소에 있는 재료에 적용한 실측치다. 같은 파일 구성으로 동결 근사 코퍼스에 K=10을 적합한 묶음이 `frozen_v7_40/`에 있으나 동결 값은 재현되지 않았다(`frozen_v7_40/FROZEN_MODEL_REFIT_V7_40.md`).\n")
...
md.append(f"| 토크나이저 | 14개 언어 라우팅판 `run_lda_v6_live_reference_v7.py` (원본 유실, 저장소 루트에 재구성본) | 같음 | `run_lda_v6.py`의 `tokenize()` (저장소 실물, 구 토크나이저) |")
...
md.append("동결 모델 묶음(①의 φ·거리 행렬·모델)을 채우는 순서는 **코퍼스 → 토크나이저 → 재적합**이다. 코퍼스는 근사 복원본(`data/v7_final/frozen_snapshot_v7_40/`, 7,326/7,350건), 토크나이저는 재구성본(저장소 루트 `run_lda_v6_live_reference_v7.py`)으로 확보됐다. `build_frozen_model_bundle_v7_40.py`가 그 둘로 K=10을 적합해 `frozen_v7_40/`에 같은 구성의 묶음을 만들었다. 검증은 "
