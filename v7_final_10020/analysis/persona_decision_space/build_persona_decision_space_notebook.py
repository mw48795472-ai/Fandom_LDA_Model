# -*- coding: utf-8 -*-
"""persona_decision_space_v7.ipynb 생성기 — Persona_결정공간.html 에 설정된 로직(K→F 배정, 덴드로그램, 상위 2 F코드 → 페르소나,
PCA 결정공간, 레이더 상세 패널)을 파이썬으로 그대로 재현하는 노트북을 만들고 실행한다. Spyder용 `# %%` 셀 스크립트도 함께 쓴다.

실행: python v7_final_10020/analysis/persona_decision_space/build_persona_decision_space_notebook.py            # 생성 + 실행
      python v7_final_10020/analysis/persona_decision_space/build_persona_decision_space_notebook.py --no-exec  # 생성만
"""
import subprocess
import sys
...
OUT = HERE / "persona_decision_space_v7.ipynb"
OUT_PY = HERE / "persona_decision_space_v7.py"
...
def write_spyder_script(cells, path):
    parts = ["# -*- coding: utf-8 -*-",
    ...
    for c in cells:
        if c["cell_type"] == "markdown":
            ...
            parts.extend("# " + line if line else "#" for line in c["source"].splitlines())
        else:
            parts.append("# %%")
...
def main(execute=True):
    ...
    write_spyder_script(cells, OUT_PY)
