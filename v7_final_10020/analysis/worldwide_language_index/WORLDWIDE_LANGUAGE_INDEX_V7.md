...
> 입력: 최종 원본 `worldwide_language_pilot_live_reference_v7.json`(+CSV)과 라이브 점수 JSON의 `coverage_detail`(14개 언어). 실행 노트북: `worldwide_language_index_v7.ipynb`.
...
1. **LanguageCoverage 공식 재검증 — 두 시점 각각의 분모** — 라이브 10,020건(14개 언어, ln(14)) 100/100 · 동결 7,350건(13개, 아랍어 없음, ln(13)) 100/100.
   다른 분모를 쓰면 100/100 불일치(`verify_v7_final_consistency.py` [V]·[Z]와 같은 결론).
...
3. **코퍼스 언어 구성 라이브·동결 비교** (팬덤별 `language_counts` 합; 라이브 값은 `language_domain_summary_v7.json`과 14개 언어 전부 일치)

| 언어 | 라이브 10,020 | 동결 7,350 |
|---|---:|---:|
| 한국어 ko | 5,551 (55.4%) | 4,183 |
| 영어 en | 2,195 (21.9%) | 1,751 |
| 일본어 ja | 520 | 333 |
| 중국어 zh | 458 | 153 |
| 스페인어 es | 250 | 121 |
| 인도네시아어 id | 210 | 193 |
...
1. 언어는 **출처 도메인** 기준이며 본문 언어가 아니다(한국 매체의 영문판, 해외 매체의 한국어판은 도메인 언어로 분류). 불릿 10,020건의 본문을 문자권·wordfreq 빈도로 따로 판정한 결과(`BODY_LANGUAGE_V7.md`, L14) 본문의 84.5%가 한국어이고 도메인 언어와 본문 언어가 같은 불릿은 49.2%다. 외국 도메인 불릿 대부분이 한국어 요약이므로 이 지수는 '출처 언어권 다양성'이고, 본문 기준으로 바꾸면 지수 평균이 0.48에서 0.17로 무너진다. 본문 판정은 보조 신호로만 둔다.
2. 14개 언어 외(독일어·이탈리아어 등)는 규칙이 없으면 기본값으로 흡수되므로 검출언어수는 하한이다. 본문 판정에서 라틴계 19개 다른 언어 후보로 잡힌 불릿은 0건이라, 이 하한의 실제 크기는 본문 기준으로는 작다(도메인 기준으로는 규칙 없는 도메인 1,556건이 기본값에 떨어진다).
3. 두 시점은 분모가 달라 language_coverage 절대치를 시점 간 비교할 수 없다.
