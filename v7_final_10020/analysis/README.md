...
| `persona_decision_space/live_interpretive_layer/` | `LIVE_INTERPRETIVE_LAYER_V7.md` + `../build_live_interpretive_layer_v7.py` — 게이트 v2 통과 모델(라이브 K=8 참고 재적합)의 상위 15·K→F·페르소나를 동결과 같은 규칙으로 만들어 대조. 게이트 v2 채택으로 루트 README 4·5절의 해석 계층이 이 폴더 값이다 |
