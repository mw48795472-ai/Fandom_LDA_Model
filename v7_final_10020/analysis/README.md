...
| `persona_decision_space/live_interpretive_layer/` | `LIVE_INTERPRETIVE_LAYER_V7.md` + `../build_live_interpretive_layer_v7.py` — 채택 해석 계층(라이브 코퍼스 r73 재적합 K=8, M=5 고정)의 상위 15·K→F·페르소나를 동결과 같은 규칙으로 만들어 대조. 루트 README 4·5절이 이 폴더 값이다 |
| `persona_decision_space/live_interpretive_layer_r72/` | 같은 스크립트를 `--r72`로 실행한 r72 라이브 K=8(게이트 v2 통과 모델) 계층 — r73 채택 전 계층의 보관본 |
