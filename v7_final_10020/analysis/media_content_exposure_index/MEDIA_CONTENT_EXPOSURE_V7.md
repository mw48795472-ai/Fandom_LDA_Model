# 미디어·콘텐츠 노출 지수 · 매체 크로스오버 지수(=매체 확산 폭 지수) — 최종 코퍼스 10,020건 판

> 이름 주의: '크로스오버'는 팬 유입 경로나 전환을 뜻하지 않는다. 이 지수가 재는 것은 한 팬덤을 서로 다른 매체가 얼마나 폭넓게 다뤘는가(매체 확산 폭)뿐이다. CSV 헤더·JSON 키는 호환을 위해 그대로 두고 문서에서만 이 별칭을 병기한다.

> 입력: 최종 원본 두 개(`media_exposure_v7.json`, `media_crossover_index_v7.json`)와 K=9 검증 파일, v7 10·11라운드 병합 로그. 실행 노트북: `media_content_exposure_v7.ipynb`.

## 두 지수의 정의

| 지수 | 정의(원문 `methodology`) | 코퍼스 합계 |
|---|---|---|
| 미디어·콘텐츠 노출 지수 | 4개 서브태그(예능/유튜브/영화/드라마) 원문 문자열 매칭. LDA 토픽·메타요인이 **아니며**, 보고서 10.8절 K9/F6 검증에서 통계적으로 독립된 미디어 토픽·요인이 확인되지 않아 도입 | 1,025건(10.2%), 99개 팬덤; 드라마 313 · 예능 309 · 유튜브 286 · 영화 179 |
| 매체 크로스오버 지수 | `source_type_of()=='news_media'`인 근거문장만 대상으로 `domain_of(u)` 기준 고유 매체 수(n_distinct_outlets)와 매체다양성비율(= 고유 매체 수 / 뉴스 불릿 수). SNS·커뮤니티·위키는 매체가 아니므로 제외 | 뉴스 불릿 6,712건(67.0%), 고유 매체 1,298개(상위 v.daum.net 377 · starnewskorea.com 251 · sports.khan.co.kr 216) |

## 노트북이 확인한 것

1. **무결성** — 두 지수 모두 근거문장 수 = 코퍼스 100/100, 비율 재계산 불일치 0, 합계(1,025 / 6,712)와 서브태그 합 = JSON.
   서브태그명 4개를 단순 포함 매칭으로 재적용하면 1,025건, 팬덤별 정확 일치 100/100 — 노출 지수는 서브태그명 자체가 키워드다.
2. **상위 팬덤** — 노출: 임영웅 27건(20.6%, 1위) · 카더가든 26(35.1%) · 오마이걸 21 · SEVENTEEN 20 · EXO 19; BTS 13건(5.7%, 28위) · 리센느 10건(11.8%).
   크로스오버 고유 매체: BTS 96개(뉴스 177건, 다양성 0.542) · TWICE 90 · BLACKPINK 83 · 지드래곤 80 · 싸이 76; 임영웅 60개(0.496) · 리센느 36개(0.529).
3. **v7 10·11라운드 교차검증** — 두 라운드 신규 근거 274건(131+143, `per_group_added` 합 일치). 신규 근거 수와 미디어문장수 Pearson r = 0.426
   (키워드 지수는 그 라운드에서 들어온 문장을 직접 세므로 양의 상관이 자연스럽다). 신규 근거 상위 10 ∩ 미디어문장수 상위 10 = 2개(이찬원·임영웅).
4. **LDA 미디어노출형(동결 스냅샷, 보고서 본문)과의 대조** — n=97, Pearson r = 0.520, Spearman ρ = 0.469. 동결에서 미디어노출형이 dominant인 팬덤: 박정현·브라운아이즈.
5. **K=9 검증(`k9_validation_v7.json`, 9,614문서 시점)** — K=9를 그리드에 넣어도 승자는 K=8(합성순위 9). K=9 phi의 M-grid에서 미디어 토픽이 단독 메타요인으로
   분리되는 것은 M=6∼8뿐이고 그때 실루엣은 0.081/0.042/0.023 → 독립 미디어 메타요인은 채택하지 않고 키워드 지수로 대체.

## 이 문서와 함께 보는 파일

- `data/v7_final/media_exposure_v7.json`, `media_crossover_index_v7.json` / `.csv`, `k9_validation_v7.json`, `data/v7_rounds/merge_log_r10.json`·`r11`.

## 한계

1. 두 지수 모두 문자열/도메인 매칭 보조지표다. 크로스오버 지수는 팬 유입 경로를 측정하지 않는다(`scope_and_limits`).
2. `domain_of()`/`source_type_of()` 정의는 최종 토크나이저 소스(저장소에 없음)에 있으므로 고유 매체 수는 원본 값을 검증 대상으로만 삼았다.
