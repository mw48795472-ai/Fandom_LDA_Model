...
md.append(f"\n합계 {len(rows)}행. 없음으로 표시된 두 사전(광고 업종 키워드 전체, 결속 유형 키워드)은 원본 산출물 JSON에 집계값만 남아 있어 복원할 수 없다. "
          "`--check` 모드가 저장 CSV와 코드·JSON의 사전을 대조해 어긋나면 실패하므로, 사전을 바꿀 때는 이 스크립트를 다시 돌려 CSV를 함께 커밋한다. 지수 스크립트(노트북 생성기)가 코드 안 사전 대신 이 CSV를 읽게 바꾸는 일은 다음 라운드 몫이다. 누락 검수(L16, `analysis/unmatched_audit/`)에서 나온 사전 추가 후보(영문 상업 신호, 예능 프로그램명·라디오, 영문 지명·군 단위)는 `dictionary_candidates_from_audit_v7.csv`에 있다 — 아직 사전에 넣지 않았다.\n")
