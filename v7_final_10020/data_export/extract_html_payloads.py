...
두 HTML은 최종 제출본과 함께 발행된 시각화 산출물이며, 원본 산출 값(덴드로그램 병합 높이,
chart3d payload)이 그 안에 그대로 들어 있다. 동결 코사인 거리 행렬 자체는 원본 데이터 묶음에도
파일로 없었으므로, HTML에 남은 병합 높이가 그 행렬의 유일한 흔적이다.
